...
| 30 | 726道八股客观自测题库 + 交互式刷题与错题斩题系统 (/quiz) | Antigravity | ✅ | 解析 726 题入库 (34专题)；QuizQuestion/QuizRecord 模型与 API；交互式刷题卡片 + 错题斩题模式 + 34专题库 + 模拟小测 + 全键盘快捷键；pytest 65 passed + vite build 通过 | 2026-08-25 |
| 31 | AI 助教（自带 key 流式代理）+ 背题对照画布 + 登录注册页重设计 | Antigravity | ✅ | ai.py 中转代理 + AiTutorDrawer/AiSettingsModal（密钥存 localStorage）；Claude 复查加固：SSRF 域名白名单 AI_ALLOWED_HOSTS + 禁跟随重定向 + test_ai；quiz 今日统计改按客户端时区；pytest 68 passed + typecheck/build 通过 | 2026-08-25 |
